...
class AttemptModelCase(unittest.TestCase):
    ...
    def test_sm2_update(self):
        ...
        q2_attempt.sm2_update(2, True) # is there a seperate function somewhere checking if it a repeat attempt?
        ...
        self.assertEqual(q2_attempt.next_attempt, date.today()) 
        self.assertEqual(q2_attempt.e_factor, 3) 
        self.assertEqual(q2_attempt.interval, 1) 
        self.assertEqual(q2_attempt.quality, 2) 
        ...
        q4_attempt.sm2_update(3) 
        db.session.add(q4_attempt)
        self.assertEqual(q4_attempt.interval,6) 
        self.assertEqual(q4_attempt.e_factor,2.36)
        self.assertEqual(q4_attempt.next_attempt,date.today() + timedelta(days=6))
        self.assertEqual(q4_attempt.quality, 3) 
        ...
        q5_attempt.sm2_update(3) 
        db.session.add(q5_attempt) 
        self.assertEqual(q5_attempt.interval,8) 
        self.assertEqual(q5_attempt.next_attempt,date.today() + timedelta(days=8))
        self.assertEqual(q5_attempt.e_factor,2.36)
        self.assertEqual(q5_attempt.quality, 3) 
