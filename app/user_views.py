...
@login_required
def review_questions(course_name, mission_id, lo_id, sequence_num):
    course = check_course_authorization(course_name)
    mission = check_mission_inclusion(mission_id, course)

    review_questions = review_questions(current_user.id, mission, 2.5) #remember to pass through the user in template
