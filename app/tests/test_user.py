...
class UserModelCase(unittest.TestCase):
    ...
    def test_get_active_courses(self):
        ...
        # to make sure that a new course isn't appended QEUSTION
        db.session.add(c5)
        db.session.commit()
    ...
    def test_get_past_courses(self):
        ...
        self.assertEqual(u.get_past_courses(), [])
        ...
        current_courses = u.get_past_courses()

        # check that contents are correct (i.e. disregarding order)
        self.assertCountEqual(current_courses, [c4])

        # check that results are sorted from oldest start_date to newest
        self.assertEqual(current_courses, [c4])
